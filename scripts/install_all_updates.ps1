...
$kbList = & sqlite3.exe "$DBPath" "SELECT DISTINCT kb FROM patches WHERE kb != '';" | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne "" }
...
foreach ($kb in $kbList) {
    ...
    try {
        # Install the update and ignore reboot for now
        ...
        # Use $($_) to safely expand the error message
        Write-Warning ("Failed to install {0}: {1}" -f $kb, $_.Exception.Message)
    }
}
